    ...
    render: function() {
        if (this.data.locatns[0] !== undefined) {
            ...
            for(var i = 0; i < this.data.locatns.length; ++i) {
                for(var j = 0; j < 3; ++j) {
                    var locatn = this.data.locatns[j];
                    var number = i;
                    var locatnAndNum = {};
                    locatnAndNum.locatn = locatn;
                    locatnAndNum.number = number;
            ...
            return (
                React.createElement("div", {className: "projectInfo"}, 
                    locatnArray.map(function(locatnThree) {
                        return React.createElement(ProjectLocatnRow, {locatnThree: locatnThree, numbers: numbers})
                    })
                )
            );
        } else {
