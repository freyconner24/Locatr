var Locatn = React.createClass({displayName: "Locatn",
    render: function() {
        function initializeMap() {
            alert("Hello");
            var myLatlng = new google.maps.LatLng(-25.363,131.044); // Add the coordinates
            var mapOptions = {
        ...
        if (this.props.locatn !== undefined) {
            ...
            console.log("Locatn::locatId: " + this.props.locatn.id);
            profileTag = (React.createElement(ProfileTag, {locatnId: this.props.locatn.id}));
        }

        google.maps.event.addDomListener(window, 'load', this.initializeMap); // Execute our 'initialize' function once the page has loaded. 
